                        ...
                        @role('user')
                            ...
                            <div class="form-group">
                                @if (Auth::user()->biodata->kategori_pemohon == 1)
                                    ...
                                    <p><a href="{{ asset(Auth::user()->biodata->file_path) }}" target="_blank"
                                            class="btn icon btn-primary"><i data-feather="file"></i> File Anda</a></p>
                                @else
                                    <label class="fw-bold">KTP Pribadi</label>
                                    <p><a href="{{ asset(Auth::user()->biodata->file_path) }}" target="_blank"
                                            class="btn icon btn-primary"><i data-feather="credit-card"></i> KTP</a></p>
                                @endif
                            </div>
                        @endrole
                    </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex align-items-center justify-content-between">
                            ...
                            @role('admin')
                                <a href="{{ route('admin.account') }}" class="btn icon btn-primary fw-bold"><i
                                        data-feather="settings"></i> Pengaturan Akun</a>
                            @endrole
                            @role('petugas')
                                <a href="{{ route('petugas.akun') }}" class="btn icon btn-primary fw-bold"><i
                                        data-feather="settings"></i> Pengaturan Akun</a>
                            @endrole
                            @role('user')
                                <a href="{{ route('user.akun') }}" class="btn icon btn-primary fw-bold"><i
                                        data-feather="settings"></i> Pengaturan Akun</a>
                            @endrole
                        </div>
                        <hr>
                        <form action="{{ route('admin.account.password.update', Auth::user()->id) }}" method="post">
                            @csrf
                            @method('put')
                            <div class="form-group">
                                <label class="fw-bold">Password Sekarang</label>
                                <input type="password" name="password"
                                    class="form-control mypassword @error('password') is-invalid @enderror">
                                @error('password')
                            ...
                            <div class="form-group">
                                ...
                                <input type="password" name="password_baru"
                                    class="form-control mypassword @error('password_baru') is-invalid @enderror">
                                @error('password_baru')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="form-group">
                                ...
                                <input type="password" name="konfirmasi_password_baru"
                                    class="form-control mypassword @error('konfirmasi_password_baru') is-invalid @enderror">
