                                    ...
                                    <div class="row">
                                        <div class="col-12">
                                            ...
                                            <div class="form-group">
                                                <select
                                                    class="choices form-select @error('permoinfo_id') is-invalid @enderror"
                                                    name="permoinfo_id" id="permoInfo">
                                                    <option value="" hidden>Pilih</option>
                                                    <option value="{{ $pengkeb->permoinfo_id }}" selected>
                                                        {{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $pengkeb->created_at)->isoFormat('dddd, D MMMM Y') }}
                                                        - {{ $pengkeb->permohonaninformasi->rincian }}</option>
                                                    @foreach ($datas as $data)
                                                        <option value="{{ $data->id }}">
                                                            @role('admin')
                                                                [{{ $data->user->name }}]
                                                            @endrole
                                                            {{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $data->created_at)->isoFormat('dddd, D MMMM Y') }}
                                                            - {{ $data->rincian }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea name="pesan" rows="5" class="form-control @error('pesan') is-invalid @enderror">{{ $pengkeb->pesan }}</textarea>
                                                @error('pesan')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
...
@section('scripts')
    @role('admin')
        <script>
            ...
            const myYear1 = new Intl.DateTimeFormat('id', {
                year: 'numeric'
            }).format(myDate1);
            const myMonth1 = new Intl.DateTimeFormat('id', {
                month: 'long'
            }).format(myDate1);
            const myDay1 = new Intl.DateTimeFormat('id', {
                day: 'numeric'
            }).format(myDate1);
            const myWeekDay1 = new Intl.DateTimeFormat('id', {
                weekday: 'long'
            }).format(myDate1);
                        ...
                        success: function(data) {
                            if (data) {
                                $.each(data, function(index, element) {
                                    ...
                                    const myYear = new Intl.DateTimeFormat('id', {
                                        year: 'numeric'
                                    }).format(myDate);
                                    const myMonth = new Intl.DateTimeFormat('id', {
                                        month: 'long'
                                    }).format(myDate);
                                    const myDay = new Intl.DateTimeFormat('id', {
                                        day: 'numeric'
                                    }).format(myDate);
                                    const myWeekDay = new Intl.DateTimeFormat('id', {
                                        weekday: 'long'
                                    }).format(myDate);
